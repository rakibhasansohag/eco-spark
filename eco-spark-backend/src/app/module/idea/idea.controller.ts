...
export const IdeaController = {
	...
	getAllForAdmin: catchAsync(async (req: Request, res: Response) => {
		const result = await IdeaService.getAllForAdmin(
			req.query as Record<string, string>,
		);
		sendResponse(res, {
			httpStatusCode: StatusCodes.OK,
	...
	getSimilar: catchAsync(async (req: Request, res: Response) => {
		const result = await IdeaService.getSimilar(req.params.id as string);
		sendResponse(res, {
			httpStatusCode: StatusCodes.OK,
			success: true,
			message: 'Similar ideas fetched',
			data: result,
		});
	}),
	...
	autoSeed: catchAsync(async (req: Request, res: Response) => {
		...
		sendResponse(res, {
			httpStatusCode: StatusCodes.CREATED,
			success: true,
			message: 'AI ideas seeded successfully',
			data: result,
		});
	}),
	...
	getById: catchAsync(async (req: Request, res: Response) => {
		const result = await IdeaService.getById(
	...
	update: catchAsync(async (req: Request, res: Response) => {
		const files = (req.files as Express.Multer.File[]) ?? [];
		const result = await IdeaService.update(
			req.params['id'] as string,
			req.user!.userId,
			req.body,
			files,
	...
	remove: catchAsync(async (req: Request, res: Response) => {
		const result = await IdeaService.remove(
			req.params['id'] as string,
			req.user!.userId,
			req.user!.role,
		);
	...
	submit: catchAsync(async (req: Request, res: Response) => {
		const result = await IdeaService.submit(
			req.params['id'] as string,
			req.user!.userId,
		);
		sendResponse(res, {
	...
	reject: catchAsync(async (req: Request, res: Response) => {
		const result = await IdeaService.reject(
			req.params['id'] as string,
			req.body,
		);
		sendResponse(res, {
